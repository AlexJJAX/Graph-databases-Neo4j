"""Configuration for the hybrid GraphRAG research assistant."""

from __future__ import annotations

import os
from dataclasses import dataclass
from pathlib import Path

from dotenv import load_dotenv


PROJECT_DIR = Path(__file__).resolve().parent
REPOSITORY_ROOT = PROJECT_DIR.parent
ANSWER_MODEL = "gpt-5.6-luna"
EMBEDDING_MODEL = "text-embedding-3-small"
EMBEDDING_DIMENSIONS = 1536
VECTOR_INDEX_NAME = "research_chunk_embedding"
FULLTEXT_INDEX_NAME = "research_chunk_fulltext"


class ConfigurationError(ValueError):
    """Raised when required environment configuration is unavailable."""


def load_repository_env() -> None:
    load_dotenv(REPOSITORY_ROOT / ".env", override=False)


def _required(name: str) -> str:
    value = os.getenv(name, "")
    if not value.strip():
        raise ConfigurationError(f"Missing required environment variable: {name}")
    return value


@dataclass(frozen=True, slots=True)
class Neo4jConfig:
    uri: str
    username: str
    password: str
    database: str = "neo4j"

    @classmethod
    def from_env(cls) -> "Neo4jConfig":
        load_repository_env()
        return cls(
            uri=_required("NEO4J_URI").strip(),
            username=_required("NEO4J_USERNAME").strip(),
            password=_required("NEO4J_PASSWORD"),
            database=os.getenv("NEO4J_DATABASE", "neo4j").strip() or "neo4j",
        )


@dataclass(frozen=True, slots=True)
class AppConfig:
    neo4j: Neo4jConfig
    openai_api_key: str
    answer_model: str = ANSWER_MODEL
    embedding_model: str = EMBEDDING_MODEL
    embedding_dimensions: int = EMBEDDING_DIMENSIONS
    minimum_semantic_score: float = 0.25

    @classmethod
    def from_env(cls) -> "AppConfig":
        load_repository_env()
        try:
            threshold = float(os.getenv("RESEARCH_MIN_SEMANTIC_SCORE", "0.25"))
        except ValueError as exc:
            raise ConfigurationError(
                "RESEARCH_MIN_SEMANTIC_SCORE must be a number"
            ) from exc
        if not 0 <= threshold <= 1:
            raise ConfigurationError(
                "RESEARCH_MIN_SEMANTIC_SCORE must be between 0 and 1"
            )
        return cls(
            neo4j=Neo4jConfig.from_env(),
            openai_api_key=_required("OPENAI_API_KEY"),
            minimum_semantic_score=threshold,
        )
